...
For this context, the Scale is considered to be a box around the position from -Scale/2 to Scale/2 in size, rounding down. Thus, Scale is a radius of this box. This box is rotated according to the current rotation. This means that each side is `Scale.n` in length
