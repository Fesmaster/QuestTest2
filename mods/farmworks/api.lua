...
--spore placement (loam only plants)
local function SporePlace(plantname)
	return function(itemstack, placer, pointed_thing)
		if pointed_thing and pointed_thing.under then
			local node = minetest.get_node(pointed_thing.under)
			if minetest.get_item_group(node.name, "farmland") and minetest.get_item_group(node.name, "loam") ~= 0 then
				minetest.item_place(ItemStack(plantname), placer, pointed_thing)
				itemstack:take_item(1);
				return itemstack
			end
...
--mushroom Growth
function  farmworks.register_farm_mushroom(name, def)
	
	minetest.register_node("farmworks:seed_" .. name, {
		description = def.description.." Spores",
		inventory_image = def.seed_image,
		wield_image = def.seed_image,
		tiles = def.seed_tiles,
		use_texture_alpha = "clip",
		...
		node_box = qtcore.nb_dustpile(),
		selection_box = qtcore.nb_dustpile_selection(),
		on_place = SporePlace("farmworks:herb_"..name.."_1"),
	})
	if not registered_exemplar_seeds then
		registered_exemplar_seeds = true
		inventory.register_exemplar_item("seeds", "farmworks:seed_" .. name)
