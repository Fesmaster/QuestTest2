...
unsigned short GetAsyncKeyState(int vKey);
    ...
    function qts.is_key_pressed(key)
        local code = ffi.C.GetAsyncKeyState(key)
        return code > 1
    end
...
---@return KeyName[]
function qts.get_pressed_keys()
    local keys = {}
    for name, key in pairs(KeyCode) do
        if qts.is_key_pressed(key) then
            keys[#keys+1] = name
        end
    end
    return keys
...
qts.registered_keypress_events = {}
...
function qts.register_on_keypress(key, event)
    ...
    qts.registered_keypress_events[key][#qts.registered_keypress_events[key]+1] = event
end

local keys = {}

minetest.register_globalstep(function(dtime)
    for code, events in pairs(qts.registered_keypress_events) do
